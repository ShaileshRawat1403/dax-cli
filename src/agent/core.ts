...
export class Agent {
  ...
  private async writeBack(result: ToolResult, toolCall: ToolCall) {
    ...
    const summary = result.success
      ? result.output.slice(0, 180)
      : result.error || "failed";
    const outcomes = [
      ...this.pmState.recent_outcomes,
      {
        ts: new Date().toISOString(),
        tool: toolCall.function.name,
      ...
      if (row.kind !== "run" || !row.run) {
        return {
          ...
          recorded:
            row.kind === "audit"
              ? row.audit || null
              : row.run || row.override || null,
          simulate: {
            available: false,
            reason: "simulation requires run event tool metadata",
          },
        };
      }
      const call: ToolCall = {
  ...
  async approvePendingOnce() {
    ...
    this.setLastAudit(
      gate.warnings.length > 0 ? "warn" : "clean",
      gate.warnings,
    );
    this.conversation.push({
      role: "assistant",
  ...
  async alwaysAllowFromPending(kind: "tool" | "path", pattern?: string) {
    if (!this.projectId || !this.pendingGate || this.pendingGate.blocked)
      return false;
    const extra =
      kind === "path"
        ? [{ kind: "path" as const, pattern: pattern || "" }].filter(
            (rule) => rule.pattern,
          )
        : this.pendingGate.toolCalls
            .map((call) => ({
              kind: "tool" as const,
              pattern: call.function.name,
            }))
            .filter((rule) => rule.pattern);
    if (extra.length === 0) return false;
    const next = [
      ...(this.pmState.constraints.always_allow || []),
      ...extra,
    ].filter(
      (row, index, arr) =>
        arr.findIndex(
          (item) => item.kind === row.kind && item.pattern === row.pattern,
        ) === index,
    );
    this.pmState = await savePM(
      this.projectId,
  ...
  async streamTask(
    ...
    // Use streaming for work notes generation
    const workNotePrompt = createWorkNotesPrompt(taskDescription);
    const msgs = await this.contextMessages([
      { role: "user", content: workNotePrompt },
    ]);

    if (typeof this.config.provider.stream !== "function") {
      // Fallback to complete if no streaming
      const response = await this.config.provider.complete(msgs, undefined, this.planningConfig());
      const content = response.content;
      onChunk(content);
      await this.parseWorkNotesFromContent(content);
      return;
    }
    ...
    try {
      ...
      await this.parseWorkNotesFromContent(buffer);
    } catch (error) {
      this.conversation.push({
        role: "assistant",
  ...
  private async parseWorkNotesFromContent(content: string): Promise<void> {
    try {
      const cleaned = content.replace(/```json\n?|\n?```/g, "").trim();
      const notes = JSON.parse(cleaned) as WorkNotes;
  ...
  private async runBuild(): Promise<boolean> {
    ...
    if (response.tool_calls && response.tool_calls.length > 0) {
      ...
      this.setLastAudit(
        gate.blocked ? "blocked" : gate.needs_approval ? "warn" : "clean",
        gate.warnings,
      );
  ...
  async chatStream(
    ...
    while (turn < MAX_TURNS) {
      ...
      const msgs = await this.contextMessages();
      if (typeof this.config.provider.stream !== "function") {
        options?.onStateChange?.("request_sent");
        const response = await this.config.provider.complete(
          msgs,
          this.config.tools.toLLMTools(),
          this.config.llmConfig,
        );
      ...
      try {
        ...
        awaitingTimer = setTimeout(() => {
          if (!sawToken && !finished) {
            options?.onStateChange?.("awaiting_first_token");
          }
        }, 150);
        ...
        while (true) {
          ...
          const budget = sawToken
            ? remaining
            : Math.min(firstTokenTimeout, remaining);
          const next = await Promise.race<
            | IteratorResult<{ content: string; tool_calls?: ToolCall[] }>
            | { timeout: true }
          >([
            it.next() as Promise<
              IteratorResult<{ content: string; tool_calls?: ToolCall[] }>
          ...
          const hasVisibleText = !!chunk.content;
          const hasToolCalls = (chunk.tool_calls?.length ?? 0) > 0;

          if (!sawToken && (hasVisibleText || hasToolCalls)) {
            sawToken = true;
        ...
        const isQuotaError = 
          ...
          errorMessage.toLowerCase().includes("terminalquotaerror");
        
        // For quota/timeout errors, don't try fallback on same provider - let outer handler switch providers
        if (isQuotaError || timeoutKind) {
          if (timeoutKind) {
            options?.onTimeout?.(timeoutKind);
          }
          // Re-throw so outer error handler can switch providers
          throw new Error(`stream ${timeoutKind || "error"}: ${errorMessage}`);
        }
        
        // For other errors, try complete() as fallback on same provider
        if (timeoutKind) {
          options?.onTimeout?.(timeoutKind);
        }
        options?.onFallback?.();
      ...
      this.messages.push({ role: "assistant", content, tool_calls: calls });

      if (!calls || calls.length === 0) {
        break; // End of multi-turn loop
      }

      const toolResults: ToolResult[] = [];
      for (const toolCall of calls) {
        options?.onStateChange?.("tool_executing", {
